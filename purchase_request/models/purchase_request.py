...
class PurchaseRequest(models.Model):
    ...
    requested_dept = fields.Many2one('hr.department', 'Requested Dept')

    assigned_to = fields.Many2one('res.users', 'Approver',  required=True, track_visibility='onchange', domain=lambda self: [('groups_id', 'in', self.env.ref('purchase_request.group_purchase_request_manager').id)])
...
#        return request
    ...
    def button_draft(self):
        for cek in self.line_ids:
            if cek.purchase_state == 'purchase'  or cek.purchase_state == 'done' :  
               raise UserError(_("You can't reject because PO already approve/done"))
