...
class bisa_FleetVehicle(models.Model):
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _name = 'bisa_fleet.vehicle'
    _description = 'Vehicle'
    _order = 'license_plate asc, acquisition_date asc'

    def _get_default_state(self):
        state = self.env.ref('bisa_fleet.bisa_fleet_vehicle_state_registered', raise_if_not_found=False)
    ...
    model_id = fields.Many2one('bisa_fleet.vehicle.model', 'Model',
        track_visibility="onchange", required=True, help='Model of the vehicle')
    brand_id = fields.Many2one('bisa_fleet.vehicle.model.brand', 'Brand', related="model_id.brand_id", store=True, readonly=False)
    log_drivers = fields.One2many('bisa_fleet.vehicle.assignation.log', 'vehicle_id', string='Assignation Logs')
    log_fuel = fields.One2many('bisa_fleet.vehicle.log.fuel', 'vehicle_id', 'Fuel Logs')
    log_services = fields.One2many('bisa_fleet.vehicle.log.services', 'vehicle_id', 'Services Logs')
    log_contracts = fields.One2many('bisa_fleet.vehicle.log.contract', 'vehicle_id', 'Contracts')
    ...
    state_id = fields.Many2one('bisa_fleet.vehicle.state', 'State',
    ...
    tag_ids = fields.Many2many('bisa_fleet.vehicle.tag', 'bisa_fleet_vehicle_vehicle_tag_rel', 'vehicle_tag_id', 'tag_id', 'Tags', copy=False)
    ...
    def _get_odometer(self):
        bisa_FleetVehicalOdometer = self.env['bisa_fleet.vehicle.odometer']
        for record in self:
            vehicle_odometer = bisa_FleetVehicalOdometer.search([('vehicle_id', '=', record.id)], limit=1, order='value desc')
            if vehicle_odometer:
    ...
    def _set_odometer(self):
        for record in self:
            if record.odometer:
                ...
                self.env['bisa_fleet.vehicle.odometer'].create(data)

    def _compute_count_all(self):
        Odometer = self.env['bisa_fleet.vehicle.odometer']
        LogFuel = self.env['bisa_fleet.vehicle.log.fuel']
        LogService = self.env['bisa_fleet.vehicle.log.services']
        LogContract = self.env['bisa_fleet.vehicle.log.contract']
        Cost = self.env['bisa_fleet.vehicle.cost']
        for record in self:
            ...
            for element in record.log_contracts:
                if element.state in ('open', 'expired') and element.expiration_date:
                    ...
                    if overdue or due_soon:
                        log_contract = self.env['bisa_fleet.vehicle.log.contract'].search([
    ...
    def _search_contract_renewal_due_soon(self, operator, value):
        ...
        self.env.cr.execute("""SELECT cost.vehicle_id,
                        ...
                        FROM bisa_fleet_vehicle_cost cost
                        LEFT JOIN bisa_fleet_vehicle_log_contract contract ON contract.cost_id = cost.id
    ...
    def create(self, vals):
        res = super(bisa_FleetVehicle, self).create(vals)
    ...
    @api.multi
    def write(self, vals):
        res = super(bisa_FleetVehicle, self).write(vals)
        if 'driver_id' in vals and vals['driver_id']:
    ...
    def create_driver_history(self, driver_id):
        for vehicle in self:
            self.env['bisa_fleet.vehicle.assignation.log'].create({
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        return self.env['bisa_fleet.vehicle.state'].search([], order=order)
    ...
    def return_action_to_open(self):
        ...
        if xml_id:
            res = self.env['ir.actions.act_window'].for_xml_id('bisa_fleet', xml_id)
            res.update(
    ...
    def act_show_log_cost(self):
        ...
        res = self.env['ir.actions.act_window'].for_xml_id('bisa_fleet', 'bisa_fleet_vehicle_costs_action')
        res.update(
    ...
    def _track_subtype(self, init_values):
        ...
        if 'driver_id' in init_values:
            return 'bisa_fleet.mt_bisa_fleet_driver_updated'
        return super(bisa_FleetVehicle, self)._track_subtype(init_values)
    ...
    def open_assignation_logs(self):
        ...
        return {
            ...
            'view_mode': 'tree',
            'res_model': 'bisa_fleet.vehicle.assignation.log',
            'domain': [('vehicle_id', '=', self.id)],
            'context': {'default_driver_id': self.driver_id.id, 'default_vehicle_id': self.id}
        }

class bisa_FleetVehicleOdometer(models.Model):
    _name = 'bisa_fleet.vehicle.odometer'
    ...
    vehicle_id = fields.Many2one('bisa_fleet.vehicle', 'Vehicle', required=True)
...
class bisa_FleetVehicleState(models.Model):
    _name = 'bisa_fleet.vehicle.state'
    _order = 'sequence asc'
    ...
    _sql_constraints = [('bisa_fleet_state_name_unique', 'unique(name)', 'State name already exists')]


class bisa_FleetVehicleTag(models.Model):
    _name = 'bisa_fleet.vehicle.tag'
...
class bisa_FleetServiceType(models.Model):
    _name = 'bisa_fleet.service.type'
    _description = 'bisa_Fleet Service Type'
...
class bisa_FleetVehicleAssignationLog(models.Model):
    _name = "bisa_fleet.vehicle.assignation.log"
    _description = "Drivers history on a vehicle"
    _order = "date_start"

    vehicle_id = fields.Many2one('bisa_fleet.vehicle', string="Vehicle", required=True)
