...
class bisa_FleetVehicleCost(models.Model):
    _name = 'bisa_fleet.vehicle.cost'
    ...
    vehicle_id = fields.Many2one('bisa_fleet.vehicle', 'Vehicle', required=True, help='Vehicle concerned by this log')
    cost_subtype_id = fields.Many2one('bisa_fleet.service.type', 'Type', help='Cost type purchased with this cost')
    ...
    parent_id = fields.Many2one('bisa_fleet.vehicle.cost', 'Parent', help='Parent cost to this current cost')
    cost_ids = fields.One2many('bisa_fleet.vehicle.cost', 'parent_id', 'Included Services', copy=True)
    odometer_id = fields.Many2one('bisa_fleet.vehicle.odometer', 'Odometer', help='Odometer measure of the vehicle at the moment of this log')
    ...
    contract_id = fields.Many2one('bisa_fleet.vehicle.log.contract', 'Contract', help='Contract attached to this cost')
    ...
    def _set_odometer(self):
        for record in self:
            ...
            odometer = self.env['bisa_fleet.vehicle.odometer'].create({
    ...
    def create(self, vals_list):
        for data in vals_list:
            ...
            if 'contract_id' in data and data['contract_id']:
                contract = self.env['bisa_fleet.vehicle.log.contract'].browse(data['contract_id'])
            ...
            if 'odometer' in data and not data['odometer']:
                ...
                del data['odometer']
        return super(bisa_FleetVehicleCost, self).create(vals_list)


class bisa_FleetVehicleLogContract(models.Model):
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _inherits = {'bisa_fleet.vehicle.cost': 'cost_id'}
    _name = 'bisa_fleet.vehicle.log.contract'
    ...
    def default_get(self, default_fields):
        res = super(bisa_FleetVehicleLogContract, self).default_get(default_fields)
        contract = self.env.ref('bisa_fleet.type_contract_leasing', raise_if_not_found=False)
        res.update({
    ...
    generated_cost_ids = fields.One2many('bisa_fleet.vehicle.cost', 'contract_id', 'Generated Costs')
    sum_cost = fields.Float(compute='_compute_sum_cost', string='Indicative Costs Total')
    cost_id = fields.Many2one('bisa_fleet.vehicle.cost', 'Cost', required=True, ondelete='cascade')
    ...
    @api.multi
    def write(self, vals):
        res = super(bisa_FleetVehicleLogContract, self).write(vals)
        if vals.get('expiration_date') or vals.get('user_id'):
            self.activity_reschedule(['bisa_fleet.mail_act_bisa_fleet_contract_to_renew'], date_deadline=vals.get('expiration_date'), new_user_id=vals.get('user_id'))
        return res
    ...
    def act_renew_contract(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref('bisa_fleet.bisa_fleet_vehicle_log_contract_view_form').id,
            'view_type': 'tree,form',
            'res_model': 'bisa_fleet.vehicle.log.contract',
    ...
    def scheduler_manage_auto_costs(self):
        ...
        VehicleCost = self.env['bisa_fleet.vehicle.cost']
        deltas = {
        ...
        contracts = self.env['bisa_fleet.vehicle.log.contract'].search([('state', '!=', 'closed')], offset=0, limit=None, order=None)
        for contract in contracts:
            ...
            while (startdate <= today) & (startdate <= contract.expiration_date):
                ...
                self.env['bisa_fleet.vehicle.cost'].create(data)
                startdate += deltas.get(contract.cost_frequency)
        return True
    ...
    def scheduler_manage_contract_expiration(self):
        ...
        for contract in nearly_expired_contracts.filtered(lambda contract: contract.user_id):
            contract.activity_schedule(
                'bisa_fleet.mail_act_bisa_fleet_contract_to_renew', contract.expiration_date,
...
class bisa_FleetVehicleLogFuel(models.Model):
    _name = 'bisa_fleet.vehicle.log.fuel'
    _description = 'Fuel log for vehicles'
    _inherits = {'bisa_fleet.vehicle.cost': 'cost_id'}

    @api.model
    def default_get(self, default_fields):
        res = super(bisa_FleetVehicleLogFuel, self).default_get(default_fields)
        service = self.env.ref('bisa_fleet.type_service_refueling', raise_if_not_found=False)
        res.update({
            'date': fields.Date.context_today(self),
...
class bisa_FleetVehicleLogServices(models.Model):
    _name = 'bisa_fleet.vehicle.log.services'
    _inherits = {'bisa_fleet.vehicle.cost': 'cost_id'}
    _description = 'Services for vehicles'

    @api.model
    def default_get(self, default_fields):
        res = super(bisa_FleetVehicleLogServices, self).default_get(default_fields)
        service = self.env.ref('bisa_fleet.type_service_service_8', raise_if_not_found=False)
        res.update({
            'date': fields.Date.context_today(self),
            'cost_subtype_id': service and service.id or False,
            'cost_type': 'services'
