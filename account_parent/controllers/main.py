...
class ExcelExportView(ExcelExport):
    ...
    def from_data(self, fields, rows):
        ...
        total_sheet = math.ceil(len(rows)/limit)
        for sheet in range(total_sheet):
            num_sheet = sheet+1
            worksheet = workbook.add_sheet('Sheet %s'%num_sheet)
...
#         line_data = data.get('report_data', [])
        ...
        row_data = [['','','','','','',], ['Company:','Target Moves:','Date from:','Date to:',], [company, move, date_from, date_to], ['','','','','','',], ['Code', 'Name', 'Type', 'Currency', 'Debit', 'Credit']]
        for line in lines:
            ...
            credit = data[5]
            # balance = data[6]
            row_data.append([code, name, type, currency, debit, credit])
