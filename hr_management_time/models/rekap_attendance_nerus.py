...
class tbl_msi_proses_rekap(models.Model):
    ...
    def action_proses(self):
        ...
        for item2 in self.env.cr.fetchall():
            ...
            if item2[10]:
              self.env.cr.execute("select tgl_jam from tbl_msi_attendance where name = %s  and tgl_jam >= %s and tgl_jam <= %s order by tgl_jam desc limit 1", (nik, item2[4], item2[9]))
